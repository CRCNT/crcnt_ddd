...
impl StoreMeta {
  pub fn parse(derive_input: &DeriveInput) -> Self {
    ...
    let store_attr = store_attr.first().map(|x| x.clone()).unwrap_or_else(|| StoreAttr { table_name: None,
                                                                                         rename:     None,
                                                                                         features:   None, });
    ...
    StoreMeta { table_name:   store_attr.table_name
                                        .unwrap_or_else(|| format!("t_{}", entity_meta.entity_ident().to_string().to_case(Case::Snake))),
